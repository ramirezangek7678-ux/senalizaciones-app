...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(usuario.rol === 'admin' ? '/admin' : '/mis-donaciones');
    } catch (err) {
    ...
    <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div className="card" style={{ width: '100%', maxWidth: '420px' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: 'var(--blanco-apagado)', fontSize: '0.9rem' }}>Accede a tu cuenta de DonaRed</p>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.25rem', color: 'var(--blanco-apagado)', fontSize: '0.9rem' }}>
          ¿No tienes cuenta?{' '}
          <Link to="/registro" style={{ color: 'var(--amarillo)', textDecoration: 'none', fontWeight: '600' }}>Regístrate</Link>
        </p>
      </div>
    </div>
  );
